...
- Bonjour 服务发现
- 支持 NSLogger.app 实时查看（默认启用）
- 支持 macOS Console.app（可选子组件）
- 支持日志分级筛选、正则匹配、断线重连
                       DESC
  ...
  # ── Console.app（默认不启用，按需集成）────────────────────────────────────
  s.subspec 'ConsoleApp' do |ca|
    ca.source_files        = 'Classes/ConsoleApp/**/*.{h,m}'
    ca.ios.frameworks      = 'Foundation', 'UIKit'
    ca.dependency 'LogWebSDK/Core'
  end
end
